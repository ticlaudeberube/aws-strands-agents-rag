...
def _stream_chat_completions(agent, settings, request: ChatCompletionRequest, user_message: str, bypass_cache: bool):
...
async def chat_completions(request: ChatCompletionRequest, bypass_cache: bool = False):
    ...
    try:
        ...
        # DIAGNOSTIC LOGGING
        logger.info(f"[CHAT_ENDPOINT] Request received:")
        logger.info(f"[CHAT_ENDPOINT] - User message: {user_message[:100]}...")
        logger.info(f"[CHAT_ENDPOINT] - force_web_search: {force_web_search} (type: {type(force_web_search)})")
        ...
        try:
            if force_web_search:
                ...
                logger.info(f"[CHAT_ENDPOINT] Web search response: {len(sources)} sources, {len(answer)} chars")
            elif bypass_cache:
                # Bypass all caches and query LLM directly (knowledge base only, no web search)
        ...
        logger.info(f"[API_RESPONSE] Before deduplication: {len(sources) if sources else 0} sources")
...
async def chat_completions_stream(request: ChatCompletionRequest):
    ...
    try:
        ...
        max_tokens = request.max_tokens or current_settings.max_tokens
        force_web_search = request.force_web_search or False
        bypass_cache = False  # Check if this is a cache bypass request (from query params or header)
