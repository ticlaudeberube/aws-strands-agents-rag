...
class EmbeddingProvider:
    @staticmethod
    def embed_text(text: Union[str, List[str]], provider: str = 'ollama', model: Optional[str] = None):
        """Unified embedding method supporting Ollama provider."""
        if provider == 'ollama':
            return EmbeddingProvider._embed_ollama(text, model)
        else:
            raise EmbeddingError(f"Unsupported embedding provider: {provider}. Only 'ollama' is supported.")
