    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get("choices") and len(data["choices"]) > 0:
                ...
                return str(response_text).strip() if response_text else ""  # type: ignore[return-value]
        ...
        logger.error("Timeout: LLM generation took >120s")
        return ""
...
def generate_responses():
    ...
    if not check_api_health():
        ...
        logger.error("   Please start: python api_server.py")
        return
    ...
    for i, question in enumerate(questions, 1):
        ...
        if response_text:
            logger.info(
                f"{i}. Response generated in {elapsed:.1f}s - {len(response_text)} characters"
            )
            qa_pairs.append(
                {
                    "question": question,
    ...
    logger.info("\n" + "=" * 70)
    logger.info("✓ Generated responses.json")
    ...
    avg_response_length = (
        total_chars_cached / total_responses_cached if total_responses_cached > 0 else 0
    )
    ...
    if len(qa_pairs) < len(questions):
        ...
        logger.info("  ✓ All questions cached successfully!")

    logger.info("\nNext step to load into response cache:")
    logger.info("  python document-loaders/sync_responses_cache.py")
