...
class PerformanceMetrics:
    ...
    def summary(self) -> str:
        ...
        cache_summary = " | ".join(
            [f"{k.replace('_cache', '')}: {'✓' if v else '✗'}" for k, v in self.cache_hits.items()]
        )
...
class StrandsRAGAgent:
    ...
    def __init__(self, settings: Settings, cache_size: int = None, embedding_cache_ttl: int = 3600):
        ...
        try:
            ...
            logger.warning(
                f"Failed to initialize response cache: {e}. Continuing without persistent caching."
            )
            self.response_cache = None

        logger.info(
            f"StrandsRAGAgent initialized with cache_size={self.cache_size}, embedding_ttl={self.embedding_cache_ttl}s"
        )
    ...
    def _generate_embedding(self, text: str) -> Tuple[List[float], bool]:
        ...
        if text in self.embedding_cache:
            ...
            if not entry.is_expired(self.embedding_cache_ttl):
                ...
                logger.debug(
                    f"Embedding cache entry expired (TTL={self.embedding_cache_ttl}s), regenerating"
                )
    ...
    def _is_question_in_scope(self, question: str) -> bool:
        ...
        if score > 0:
            logger.debug(
                f"Scope check (keyword partial, score={score}): '{question[:50]}...' -> YES"
            )
            return True
        ...
        try:
            ...
            if distances:
                ...
                source_relevance = max(
                    0.0, 1.0 - avg_distance
                )  # Invert: 0 distance = 1.0 relevance
    ...
    def _detect_comparative_question(self, question: str) -> Tuple[bool, Optional[Tuple[str, str]]]:
        ...
        try:
            ...
            if (
                classification.get("is_comparison")
                and classification.get("product1")
                and classification.get("product2")
            ):
    ...
    def search_comparison(
        ...
        try:
            ...
            if comparison_data.get("product1") or comparison_data.get("product2"):
                logger.info(
                    "[WEB_SEARCH_COMPARISON] Processing product-specific web search results"
                )
                ...
                for feature, results in list(p1_features.items())[:2]:
                    for idx, web_result in enumerate(results[:1], 1):  # Only 1 result per feature
                        ...
                        web_source = {
                            ...
                            "title": title,
                            "distance": 0.92
                            - (web_search_count * 0.03),  # High relevance for web results
                        }
            ...
            if web_search_count > 0:
                logger.info(
                    f"✓ Added {web_search_count} web search results ({time.time() - web_search_time:.1f}s)"
                )
            else:
    ...
    def retrieve_context(
        ...
        try:
            ...
            high_quality_results = [
                r for r in search_results if r.get("distance", 0) <= SIMILARITY_THRESHOLD
            ]

            if len(high_quality_results) < len(search_results):
                filtered_count = len(search_results) - len(high_quality_results)
                logger.info(
                    f"Filtered out {filtered_count} low-quality matches (distance > {SIMILARITY_THRESHOLD})"
                )
            ...
            if not context:
                logger.warning(
                    "⚠️  No context chunks retrieved! Collection may be empty or embedding mismatch."
                )
            else:
    ...
    def generate_answer(
        ...
        try:
            ...
            logger.debug(
                f"LLM Generation Params: temperature={use_temperature}, max_tokens={use_max_tokens}"
            )
        ...
        try:
            ...
            for doc in documents:
                if isinstance(doc, str):
                    ...
                    normalized_docs.append({"text": doc, "source": "sample", "metadata": {}})
    ...
    def search_multiple_collections(
        ...
        try:
            ...
            if all_sources:
                ...
                logger.info(
                    f"Merged {len(all_chunks)} results from {len(collections)} collections, kept top {len(merged_chunks)}"
                )
    ...
    def answer_question(
        ...
        try:
            ...
            logger.debug(
                f"[CACHE_DEBUG] Cache key: question='{question[:40]}...', collections={collections}, top_k={top_k}"
            )
            ...
            logger.debug(
                f"Embedding check took {embed_time:.2f}s"
                + (" (cached)" if embedding_cached else "")
            )
            ...
            if self.response_cache and question_embedding is not None:
                ...
                if cached_response:
                    ...
                    logger.info(
                        "✓ Used cached answer - NO web search needed (answer found from pre-generated Q&A)"
                    )
                    return result_tuple
                else:
    ...
    def answer_question_no_cache(
        ...
        try:
            ...
            total_time = time.time() - start_time
            logger.info(
                f"Total response time (no cache): {total_time:.2f}s (retrieval: {retrieval_time:.2f}s, generation: {generation_time:.2f}s)"
            )
    ...
    def answer_question_web_search_only(
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    "[WEB_SEARCH_ONLY] About to call self.web_search.search() with max_results=5"
                )

                web_results = self.web_search.search(query=web_search_query, max_results=5)
                logger.info(
                    f"[WEB_SEARCH_ONLY] web_results type: {type(web_results)}, value: {web_results}"
                )
                logger.info(
                    f"[WEB_SEARCH_ONLY] Returned {len(web_results) if web_results else 0} web results"
                )
                ...
                if web_results:
                    ...
                    logger.info(
                        f"✓ Web search completed in {web_search_time:.2f}s - {len(sources)} sources appended"
                    )
                else:
                    logger.warning(
                        f"[WEB_SEARCH_ONLY] ⚠️ Web search returned NO RESULTS for query: '{web_search_query[:60]}'"
                    )
                    logger.warning(
                        "[WEB_SEARCH_ONLY] ⚠️ This usually means TAVILY_API_KEY is not set. Check your environment variables."
                    )
                    logger.warning(
                        "[WEB_SEARCH_ONLY] ⚠️ To enable web search: export TAVILY_API_KEY='your-key-here'"
                    )
            ...
            if sources:
                ...
                logger.info(
                    f"[WEB_SEARCH_ONLY] ✓ Using {len(sources)} web search results as context"
                )
            else:
                web_context = "No web search results found."
                logger.warning(
                    "[WEB_SEARCH_ONLY] ⚠️ No web search results available. Check that TAVILY_API_KEY is set."
                )
            ...
            logger.info(
                f"[WEB_SEARCH_ONLY] Final response: {len(sources)} sources, {len(answer)} chars"
            )
    ...
    async def stream_answer(
        ...
        try:
            ...
            if is_comparative and products:
                logger.info(
                    f"Comparative question detected (STREAMING): {products[0]} vs {products[1]}"
                )
                try:
                    comparison_result, comparison_sources = self.search_comparison(
                        products[0],
                    ...
                    logger.info(
                        f"Comparison streaming completed with {len(comparison_sources)} sources"
                    )
                    return
                except Exception as e:
                    logger.warning(f"Comparison search failed: {e}. Falling back to standard RAG.")
            ...
            try:
                ...
                logger.info(
                    f"Stream generation completed: {chunk_count} raw chunks -> buffered for UI"
                )
