...
class MilvusResponseCache:
    ...
    def _validate_cached_answer_relevance(
        self, question: str, cached_answer: str, cached_question: str
    ) -> bool:
        ...
        logger.debug(
            f"[CACHE_VALIDATE] Validating - current entity: {current_entity}, cached entity: {cached_entity}"
        )
        ...
        if not current_entity or not cached_entity:
            logger.debug(
                "[CACHE_VALIDATE] Could not extract entities - allowing cache hit (conservative)"
            )
            return True
        ...
        if cached_entity_lower not in answer_lower:
            logger.debug(
                f"[CACHE_VALIDATE] Different entity: cached answer for '{cached_entity}' doesn't mention it - rejecting"
            )
            return False
        ...
        if current_entity_lower in answer_lower:
            logger.debug(
                f"[CACHE_VALIDATE] Different entity: Answer mentions '{current_entity}' but is cached for '{cached_entity}' - rejecting"
            )
            return False

        logger.debug(
            "[CACHE_VALIDATE] Different entity but answer doesn't mention current entity - allowing cache"
        )
        return True
    ...
    def search_cache(
        ...
        try:
            ...
            logger.debug(
                f"[CACHE_DEBUG] Best match distance: {distance:.4f}, type: {type(distance)}"
            )
            ...
            if distance >= self.distance_threshold:
                ...
                is_relevant = self._validate_cached_answer_relevance(
                    question, cached_answer, cached_question
                )
                if not is_relevant:
